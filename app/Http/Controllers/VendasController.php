...
class VendasController extends Controller
{
          ...
          if ($request->formtype=="car") {
            ...
            if ($duplicate) {
              ...
              $duplicate->update(['quantidade'=> $quantidadeNova]);
                ...
                $entrada_id = null;
                $entradas = DB::table('produtos_venda_view')
                            ->where('id',$value)
                            ->where('status',1)
                            ->latest()
                            ->first();
                $entrada_id = isset($entradas->entrada_id) ? $entradas->entrada_id : null;
    ...
    function data_mesa($mesa_id, $filter = 'mesa_id' )
    {
        return VendasTempMesa::where($filter,$mesa_id)->whereNull('codigo_venda')
              ...
              ->select('produtos_venda_view.name','vendas_temp_mesa.quantidade','produtos_venda_view.preco_final','vendas_temp_mesa.id','vendas_temp_mesa.identificador_de_bulk')
              ->orderBy('vendas_temp_mesa.created_at','desc')
              ->get();
    }
    ...
    function data_ultima_venda($pagamento, $filter = 'vendas_temp_mesa.codigo_venda' )
    {
        return VendasTempMesa::where($filter,$pagamento)
            ...
            ->select('produtos_venda_view.codigoproduto','produtos_venda_view.name','vendas_temp_mesa.quantidade','produtos_venda_view.preco_final','vendas_temp_mesa.id','vendas_temp_mesa.identificador_de_bulk','cliente.nome','cliente.apelido','cliente.nuit','vendas_temp_mesa.codigo_venda')
            ->orderBy('vendas_temp_mesa.created_at','desc')
            ->get();
    }
